...
class EventChecker
  ...
  def run
    ...
    puts "🗓️  Check #{ONGOING_FILE} for ongoing events (>30 days)"
    ...
    if @new_ongoing_events && @new_ongoing_events.any?
      ...
      @new_ongoing_events.first(5).each do |event|
        event_type = event[:event_type] ? "#{event[:event_type]}: " : ""
        puts "   • #{event_type}#{event[:title]}"
      end
      if @new_ongoing_events.length > 5
        puts "   ... and #{@new_ongoing_events.length - 5} more"
      end
    else
  ...
  def get_date_range
    ...
    # Get end date
    print "End date (YYYY-MM-DD) or press Enter for 7 days from start: "
  ...
  def extract_krakow_event_data_simple(container, start_date, end_date)
    begin
      ...
      if is_permanent
        ...
        event_end_date = start_date + 3650  # Long duration for categorization
  ...
  def scrape_karnet_events(start_date, end_date)
    ...
    begin
      ...
      first_url = "#{WEBSITES[:karnet][:base_url]}/wydarzenia?query=&date_start=#{start_date.strftime('%Y-%m-%d')}&date_end=#{end_date.strftime('%Y-%m-%d')}&choosenArea=&radius=1000&category_id=&Item_page=1"
      ...
      (1..total_pages).each do |page|
        url = "#{WEBSITES[:karnet][:base_url]}/wydarzenia?query=&date_start=#{start_date.strftime('%Y-%m-%d')}&date_end=#{end_date.strftime('%Y-%m-%d')}&choosenArea=&radius=1000&category_id=&Item_page=#{page}"
    ...
    begin
      ...
      if response.code == '200'
        ...
        event_links = doc.css('a[href*="/"][href*="-krakow-"]').select do |link|
          ...
          href.match?(/\/\d+-krakow-/) && text.length > 10 &&
          !href.include?('wydarzenia/') && !text.include?('Zobacz wszystkie') &&
          !text.include?('Moje wydarzenia') && !text.include?('Szukaj')
        end.length
        
        events_per_page = 12  # Karnet consistently shows 12 unique events per page
  ...
  def extract_karnet_event_data_simple(container, start_date, end_date)
    begin
      ...
      title = title_element.text.strip
      return nil if title.empty? || title.length < 3
  ...
  def detect_exhibition_duration(title, container, start_date)
    ...
    if container_text.include?('wystawa stała') || 
       container_text.include?('wystawy stałe') ||
       container_text.include?('wydarzenie stałe')
      return start_date + 3650  # 10 years for permanent
    elsif title.match?(/\b(wystawa|ekspozycja|exhibition)\b/i) || container_text.include?('wystawy czasowe')
      return start_date + 120   # 4 months for temporary exhibitions  
    else
  ...
  def filter_events_by_date(start_date, end_date)
    ...
    if filtered_out.any?
      ...
      filtered_out.first(10).each { |event| puts "   - #{event}" }
      puts "   ... and #{filtered_out.length - 10} more" if filtered_out.length > 10
    end
  end
  ...
  def categorize_events(start_date, end_date)
    ...
    @ongoing_events = []      # Events longer than 30 days (go to ongoing.md)
    @multiday_events = []     # Events 1-30 days (go to upcoming.md)
    ...
    @events.each do |event|
      ...
      duration_days = (event_end - event_start).to_i
    ...
    puts "   📊 Ongoing: #{@ongoing_events.length}, Multi-day: #{@multiday_events.length}, Daily: #{@daily_events.values.flatten.length}"
  end
  ...
  def build_ongoing_content
    ...
    content += "_Events lasting longer than 30 days, organized by type_\n\n"
  ...
  def build_upcoming_content
    ...
    # Note: Ongoing events (>30 days) are now in ongoing.md
  ...
  def is_permanent_event?(event)
    return false unless event[:date_start] && event[:date_end]
    (event[:date_end] - event[:date_start]).to_i > 90
  end

  def is_multiday_event?(event)
    return false unless event[:date_start] && event[:date_end]
    duration = (event[:date_end] - event[:date_start]).to_i
